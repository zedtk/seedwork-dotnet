# Seedwork .NET - Nx Monorepo# Seedwork .NET - Nx Monorepo# SeedworkDotnet
...
---## 📋 Prérequis✨ Your new, shiny [Nx workspace](https://nx.dev) is ready ✨.

## 📚 Documentation

| Guide | Description |- [.NET SDK 8.0+](https://dotnet.microsoft.com/download)[Learn more about this workspace setup and its capabilities](https://nx.dev/getting-started/intro#learn-nx?utm_source=nx_project&utm_medium=readme&utm_campaign=nx_projects) or run `npx nx graph` to visually explore what was created. Now, let's get you up to speed!

|-------|-------------|

| **[🚀 Getting Started](GETTING_STARTED.md)** | Guide complet pour démarrer et créer vos bibliothèques |- [Node.js 20+](https://nodejs.org/)

| **[🏗️ Project Structure](PROJECT_STRUCTURE.md)** | Architecture détaillée du workspace |

| **[✨ Best Practices](BEST_PRACTICES.md)** | Standards de code et conventions |- [GitVersion Tool](https://gitversion.net/) (optionnel, installé automatiquement)## Run tasks

| **[🎉 Setup Complete](SETUP_COMPLETE.md)** | Récapitulatif de la configuration |

---

## 🚀 Démarrage rapideTo run tasks with Nx use:

## ⚡ Démarrage rapide

````bash

# Installer les dépendances### Installation```sh
...
# Exécuter tous les testsnpm install
...
### Installer GitVersion (optionnel pour le développement local)```sh

Les packages sont générés dans `dist/packages/`.
...
- **Graphe de dépendances** : Visualisation et gestion automatique

- **Exécution parallèle** : Build et test de plusieurs projets simultanément

- **Affected commands** : Ne rebuild que ce qui a changé

## 📦 Structure du projet[More about running tasks in the docs &raquo;](https://nx.dev/features/run-tasks?utm_source=nx_project&utm_medium=readme&utm_campaign=nx_projects)

### 📦 Gestion de packages

- **Versioning automatique** : GitVersion + Conventional Commits

- **NuGet prêt** : Métadonnées et symboles configurés

- **Multi-targeting** : Support .NET 9.0 (configurable)```## Add new projects

- **Documentation** : Génération automatique de XML docs
...
- **Publication automatique** : Déploiement sur NuGet.org│ └── core/ # Package exemple

- **Artefacts** : Packages sauvegardés pour chaque build

│ ├── src/ # Code sourceTo install a new plugin you can use the `nx add` command. Here's an example of adding the React plugin:

### 🎨 Developer Experience

- **Scripts PowerShell** : Build, test, pack, publish│ │ ├── Seedwork.Core.csproj```sh

- **Documentation complète** : Guides et exemples

- **Structure cohérente** : Organisation claire des projets│ │ └── Entity.csnpx nx add @nx/react
...
## 📦 Exemple : Seedwork.Core

│ └── project.json # Configuration NxUse the plugin's generator to create new projects. For example, to create a new React app or library:

Le workspace inclut une bibliothèque exemple complète :

├── tools/ # Scripts de build/test/pack

````csharp

using Seedwork.Core;│   ├── build.ps1```sh
...
{│   ├── pack.ps1npx nx g @nx/react:app demo
    ...
    public Order(Guid id, decimal total) : base(id)├── .github/workflows/         # CI/CD GitHub Actions# Generate a library
...

    {
        ...
        Status = OrderStatus.Paid;└── nx.json                    # Configuration NxYou can use `npx nx list` to get a list of installed plugins. Then, run `npx nx list <plugin-name>` to learn about more specific capabilities of a particular plugin. Alternatively, [install Nx Console](https://nx.dev/getting-started/editor-setup?utm_source=nx_project&utm_medium=readme&utm_campaign=nx_projects) to browse plugins and generators in your IDE.
...
// Utiliser[Learn more about Nx plugins &raquo;](https://nx.dev/concepts/nx-plugins?utm_source=nx_project&utm_medium=readme&utm_campaign=nx_projects) | [Browse the plugin registry &raquo;](https://nx.dev/plugin-registry?utm_source=nx_project&utm_medium=readme&utm_campaign=nx_projects)
...
seedwork-dotnet/```bash

├── packages/                   # Vos bibliothèques .NET

│   └── core/                  # Exemple : Seedwork.Core# Compiler tous les projetsTo connect to Nx Cloud, run the following command:
